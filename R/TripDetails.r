TripDetails <- function(curryear,
                        ...
                        # Maximum length of retention (by stock)
                        length.min.legal.all,               
                        # Minimum length of retention (by stock)
                        length.weight.params,
                        lengths,                            
                        # Length bins for dynamic stocks
                        modeled.stock.rec.catch.source,     
                        # Mapping of sub-area stocks to dynamic models
                        NAL,
                        noncompliance.rate,                 
                        # Rate of non-compliance with retention limits
                        num.keep.legal.all,                 
                        # Max number of fish retained by stock
                        num.keep.legal.group.by.area,       
                        # Max number of fish retained by species group
                        opt.out.prob.by.area,
                        rec.sel.at.length,                  
                        # Recreational selectivity at length
                        release.mortality.rate,             
                        # Release mortality rate by stock
                        stocks.model.pop.this.area,
                        stocks.rec.catch.by.area,           
                        # Names of the dynamic stocks
                        trip.cost.by.area,
                        trip.type.utility.coefs,
                        trip.types.by.area,                 
                        # Trip options (bottomfish, salmon) by subarea
                        utility.catch.weight.coefs,
                        utility.coefs,                      
                        # Coefficients for utility model
                        utlity.catch.squared.term.names,    
                        # Categories for quadratic utility terms
                        utlity.catch.term.names,            
                        # Categories for linear utility terms
    ...
    # Get the names of the sub-area stocks that correspond to this dynamic stock 
    # to get the selectivity info
    ...
    # Calculate numbers available to the recreational fishery to adjust catch 
    # per trip probabilities
    ...
    new.data <- data.frame(BoatType=factor(boattype, levels=boat.types), 
        ...
        LingcodAvailable=num.available.to.rec.fishery$WA.lingcod)
    lingcod.probs <- predict(wa.lingcod.salmon.catch.per.trip.model, new.data, 
        type='prob')
    catch.per.trip.prob.subsetted <- c(catch.per.trip.prob.subsetted, 
        list(WA.lingcod=lingcod.probs))
